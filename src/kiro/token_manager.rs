...
impl MultiTokenManager {
    ...
    pub fn available_count_for_request(
        ...
        self.entries
            .lock()
            .iter()
            .filter(|entry| self.entry_available_for_request(entry, model, group, now))
            .count()
    }
        ...
        match mode {
            "balanced" => {
                ...
                let (entry, _) = available
                    ...
                    .min_by_key(|(e, support)| {
                        ...
                        (discovery_rank, e.load_balance_count, e.credentials.priority)
                    })?;
            ...
            "least_conn" => {
                let (entry, _) = available.iter().min_by_key(|(e, support)| {
                    let discovery_rank = usize::from(*support != CachedModelSupport::Confirmed);
                    (discovery_rank, e.in_flight, e.credentials.priority, e.load_balance_count)
                })?;
        ...
        loop {
            ...
            let (id, credentials, is_balanced) = {
                ...
                let (id, credentials) = if let Some(hit) = current_hit {
                    ...
                    if best.is_none()
                        && !self.get_disable_failure_auto_recovery()
    ...
    ///
    /// 这不是永久失败，也不是账号级硬冷却：保留短软冷却；未开启“不限并发”时
    /// 还会动态收缩并发，让调度器优先避开该账号。
    ...
    pub fn report_failure_for_request(
        ...
        let (result, newly_disabled) = {
            ...
            entries[entry_index].last_used_at = Some(now.clone());
            ...
            if failure_count >= MAX_FAILURES_PER_CREDENTIAL {
                ...
                entries[entry_index].disabled_at = Some(now);
                entries[entry_index].clear_self_heal_streak();
                disabled_now = true;
    ...
    pub(crate) fn report_temporary_cooldown_for_request(
        ...
        entries
            .iter()
            .filter(|entry| schedulable(entry, throttled_now, model, group))
            .count()
    }
...
mod tests {
    ...
    #[test]
    fn account_throttle_behavior_flags_persist_to_config_file() {
        let config_path = std::env::temp_dir().join(format!(
            "kiro-account-throttle-{}.json",
            uuid::Uuid::new_v4()
        ));
    ...
    #[tokio::test]
    async fn failure_auto_recovery_can_be_disabled() {
        let mut config = Config::default();
        config.disable_failure_auto_recovery = true;
        ...
        let error = manager
            ...
            .await
            .err()
            .expect("关闭自动自愈后应保持全部凭据禁用");
        assert!(error.to_string().contains("所有凭据均已禁用"));
        assert_eq!(manager.available_count(), 0);
    }

    #[test]
    fn unlimited_concurrency_clears_and_ignores_dynamic_limit() {
        let manager = MultiTokenManager::new(
            Config::default(),
            vec![KiroCredentials::default()],
            None,
