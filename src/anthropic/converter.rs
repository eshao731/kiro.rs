...
pub fn map_model(model: &str) -> Option<String> {
    ...
    let normalized = normalize_claude_model(model).unwrap_or_else(|| model.to_string());
    // Anthropic 文档里出现过 Fable 5，但 Kiro 实测仍返回 INVALID_MODEL_ID。
    // 自定义模型映射在上方已优先处理，因此运维显式配置后仍可开放。
    if normalized.eq_ignore_ascii_case("claude-fable-5") {
        None
    } else {
        Some(normalized)
    }
...
fn model_supports_xhigh_effort(model_id: &str) -> bool {
    ...
    // Newer reasoning-capable model families accept xhigh.
...
mod tests {
    ...
    fn test_map_model_opus_5() {
        ...
        assert_eq!(
            map_model("claude-opus5-preview"),
            Some("claude-opus-5".to_string())
        );
    ...
    #[test]
    fn test_map_model_fable_5_not_exposed_until_kiro_supports_it() {
        assert!(map_model("claude-fable-5").is_none());
        assert!(map_model("claude-fable-5-thinking").is_none());
        assert!(map_model("claude-fable-5-20260615").is_none());
    }
